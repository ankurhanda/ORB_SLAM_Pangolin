    ...
    mbResetRequested(false)
{
    ...
    if(k3==0)
        ...
        DistCoef = (cv::Mat_<float>(5,1) << k1, k2, p1, p2, k3);
...
bool Tracking::CheckResetFromGUI()
{
    bool bReset = false;

    {
        boost::mutex::scoped_lock lock(mMutexReset);
        bReset = mbResetRequested;
    }

    if(bReset)
    {
        cout << "reset request by user" << endl;
        Reset();
